...
def chinese_remainder_theorem(a, m):
    #这个是a都为0情况， x应为所有m的最大公倍数。其实就是结果不互素的一种特例罢了
    for a_i in a:
        if a_i == 0:
            return lcm_list(m)
    M = 1
...
def get_prime(bit_length):
    '''获取一个指定二进制长度的素数'''
    rand = randint(2**(bit_length - 1), 2**(bit_length) - 1)
    # 检验(p-1)/2， 来检验rand是否是安全素数，来增加分解N的困难性
    while not is_prime(rand): # or not is_prime((rand-1)/2):  这是安全素数的标准
        rand = next_prime(rand)  # 重新选随机数效率低，直接在附近开始寻找
    return rand
